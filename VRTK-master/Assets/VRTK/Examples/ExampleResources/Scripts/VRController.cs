namespace VRTK.Examples
{
    ...
    public class VRController : MonoBehaviour
    {
        ...
        {
            print("this is x (the lower button)");
            //this is x (the lower button
            DebugLogger(VRTK_ControllerReference.GetRealIndex(e.controllerReference), "BUTTON ONE", "pressed down", e);
            GameObject.Find("OculusVR").transform.Translate(0, 1, 0);
            

            GameObject.FindGameObjectWithTag("GameController").GetComponent<GraphController>().Mode = GameObject.FindGameObjectWithTag("GameController").GetComponent<GraphController>().Mode + 1;
            if (GameObject.FindGameObjectWithTag("GameController").GetComponent<GraphController>().Mode > 5)
            {
                GameObject.FindGameObjectWithTag("GameController").GetComponent<GraphController>().Mode = 0;
            }
