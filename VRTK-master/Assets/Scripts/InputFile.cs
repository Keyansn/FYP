...
public class InputFile : MonoBehaviour
{
    ...
    public void LoadInputFile(string xmlFile)
    {
        ...
        foreach(XmlNode node in nodeList)
        {
			...
			if (x.InnerXml == "" || y.InnerXml == "" || z.InnerXml == "") {
				clone.transform.position = new Vector3(Random.Range(-scalesize, scalesize), Random.Range(-scalesize, scalesize)+10f, Random.Range(-scalesize, scalesize));
			}
			else{
            ...
            clone.GetComponent<EigenvectorCentrality>().data = data.InnerText.ToString();
